...
//	vector<string>			parameters;			//the Parameters for the function stored as strings
	...
	parameters = < < -3, 4 >, < 3, result_of_rand_fuction>, < -4, 2>, < window_width, 3>, < 1, 0.8 > >
	...
	5) when there's relative values eg.1r, set the first parameter in the pair -4 and second as the parameter of r
	...
	7) when there's a absolute value, then set the fist number in the pair"1" and second as the absolute value.
	...
	parameter = < < -4, 1>, < building_height, 1>, < -4, 0.9> >
